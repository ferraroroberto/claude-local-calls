...
def test_build_command_uses_the_comfyui_venv_and_row_port(flux_model):
    cmd = bp.build_command(flux_model)
    assert cmd[0] == str(bp.comfyui_python())
    assert cmd[1].endswith("main.py")
    assert "--port" in cmd and str(flux_model.port) in cmd


def test_build_command_binds_loopback_only(flux_model):
